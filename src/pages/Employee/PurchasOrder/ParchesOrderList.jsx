...
import {
  ...
  getPurchaseOrderListForEmp,
  updatePurchaseOrderStatusByEmp,
} from "../../../api/api";
import UniqueCode from "../../../assets/common/UniqueCode";
import { useDebounce } from "../../../hooks/useDebounce";
import { MdDownloading } from "react-icons/md";
import { getPurchaseOrderExcelViewForEmp } from "../../../api/configApi";

export const ParchesOrderList = () => {
  ...
  async function fetchDistributors() {
    try {
      setPageLoading(true);
      const res = await getEmployeeDetailsById(userInfo?._id);
      console.log("fetchDistributors->res", res);
      setDistributorList(res?.data?.data?.distributorId);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  let fetchPurchaseOrderPaginatedWithOutDebounce = async () => {
    try {
      ...
      const response = await getPurchaseOrderListForEmp(query);
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      const allIds = purchaseOrderList.map((po) => po._id);
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      const firstResponse = await getPurchaseOrderExcelViewForEmp({
        page: 1,
        limit: 20,
        ...query,
      });
      ...
      while (isFetching && currentPage < totalPages) {
        ...
        await sleep(300);
        const response = await getPurchaseOrderExcelViewForEmp({
          page: currentPage,
          limit: 20,
          ...query,
        });
      ...
      <div className="flex justify-between w-full items-center py-1">
        <div className="flex justify-start items-center w-full">
          <Breadcrumb aria-label="Solid background breadcrumb example">
            <Breadcrumb.Item>Purchase Browser</Breadcrumb.Item>
                ...
                <>
                  {purchaseOrderList?.map((po) => (
                    ...
                    >
                      ...
                      <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-gray-200">
                        {moment(po?.updatedAt).tz("Asia/Kolkata").format("LLL")}
                      </Table.Cell>
                      <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-gray-200">
                        {po?.updatedByType === "Distributor" &&
                          po?.updatedBy?.name &&
                          po?.updatedBy?.dbCode &&
                          `${po.updatedBy.name} - `}
                        <UniqueCode
                          text={po?.updatedBy?.dbCode}
                          codeName="Distributor Code"
                        />
                        {po?.updatedByType === "Employee" &&
                          po?.updatedBy?.name &&
                          po?.updatedBy?.empId &&
                          `${po.updatedBy.name}(${po.updatedBy.desgId.name}) - `}
                        <UniqueCode
                          text={po?.updatedBy?.empId}
                          codeName="Employee ID"
                        />
                        {po?.updatedByType === "User" &&
                          po?.updatedBy?.name &&
                          `${po.updatedBy.name}`}
                      </Table.Cell>
                      <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-gray-200">
                      ...
                      <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-gray-200">
                        {po?.approvedByType === "Distributor" &&
                          po?.approved_by?.name &&
                          po?.approved_by?.dbCode &&
                          `${po.approved_by.name} - ${po.approved_by.dbCode}`}
                        {po?.approvedByType === "Employee" &&
                          po?.approved_by?.name &&
                          po?.approved_by?.empId &&
                          `${po.approved_by.name}(${po.updatedBy.desgId.name}) - ${po.approved_by.empId}`}
                        {po?.approvedByType === "User" &&
                          po?.approved_by?.name &&
                          `${po.approved_by.name}`}
                      </Table.Cell>
                      <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-gray-200">
...
export default ParchesOrderList;
