...
import {
  getPortalLockCheckCron,
  updatePortalLockCheckCron,
} from "../../../../api/configApi";

const PortalLockCheckingCron = ({ canUpdate, refreshKey }) => {
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [cronConfig, setCronConfig] = useState({
    cronTime: "",
    isActive: false,
    jobName: "portalLockCheck",
  });
  ...
  const fetchCronConfig = async () => {
    ...
    setCronError("");
    try {
      const response = await getPortalLockCheckCron();
      const data = response?.data?.data || {};

      const newState = {
        cronTime: data?.cronTime || "0 23 * * *",
        isActive: typeof data?.isActive === "boolean" ? data.isActive : true,
        jobName: data?.job || "portalLockCheck",
      };

      setCronConfig(newState);
      setInitialState(newState);
    } catch (error) {
      console.error("Error fetching portal lock check cron config:", error);
      toast.error(
        error?.response?.data?.message ||
          "Failed to load portal lock check cron settings",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const response = await updatePortalLockCheckCron(payload);
      ...
      setCronConfig(newState);
      setInitialState(newState);
      toast.success("Portal lock check cron settings updated successfully");
    } catch (error) {
      const message =
        error?.response?.data?.message ||
        error?.message ||
        "Failed to update portal lock check cron";
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="font-mono text-xs text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 px-3 py-2.5 rounded-lg truncate">
                    {cronConfig.jobName || "portalLockCheck"}
                  </p>
                </div>
                <div>
                  <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1.5">
...
export default PortalLockCheckingCron;
