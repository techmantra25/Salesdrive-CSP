...
import {
  getPartiallyDeliveredBillRetryCron,
  updatePartiallyDeliveredBillRetryCron,
} from "../../../../api/configApi";

const PartiallyDeliveredBillRetryCron = ({ canUpdate, refreshKey }) => {
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [cronConfig, setCronConfig] = useState({
    cronTime: "",
    isActive: true,
    jobName: "partiallyDeliveredBillRetry",
  });
  ...
  const fetchCronConfig = async () => {
    ...
    setCronError("");
    try {
      const response = await getPartiallyDeliveredBillRetryCron();
      const data = response?.data?.data || {};

      const newState = {
        cronTime: data?.cronTime || "30 01 * * *",
        isActive: typeof data?.isActive === "boolean" ? data.isActive : true,
        jobName: data?.job || "partiallyDeliveredBillRetry",
      };
      ...
      console.error(
        "Error fetching partially delivered bill retry cron config:",
        error,
      );
      toast.error(
        error?.response?.data?.message ||
          "Failed to load partially delivered bill retry cron settings",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const response = await updatePartiallyDeliveredBillRetryCron(payload);
      ...
      setInitialState(newState);
      toast.success(
        "Partially delivered bill retry cron settings updated successfully",
      );
    } catch (error) {
      const message =
        error?.response?.data?.message ||
        error?.message ||
        "Failed to update partially delivered bill retry cron";

      if (
        error?.response?.status === 400 &&
        /(invalid cron expression|cronTime must be a non-empty string)/i.test(
          message,
        )
      ) {
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <span className="font-semibold text-sm text-gray-800 dark:text-white">
            Partially Delivered Bill Retry
          </span>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="font-mono text-xs text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 px-3 py-2.5 rounded-lg truncate">
                    {cronConfig.jobName || "partiallyDeliveredBillRetry"}
                  </p>
                </div>
                <div>
                  <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1.5">
              ...
              <div>
                ...
                <input
                  id="partiallyDeliveredBillRetryCronTime"
...
export default PartiallyDeliveredBillRetryCron;
