...
import { addBrand, BrandbulkUpload, updateBrand } from "../../api/api";
...
const Brand = () => {
  ...
  useEffect(() => {
    ...
    const slug = "brand"; // must match backend slug
  ...
  if (selectedStatus !== "default") {
    filteredBrands = [...filteredBrands]?.filter(
      (brand) => brand.status === (selectedStatus === "active" ? true : false)
    );
  }
  ...
  if (searchTerm.trim() !== "") {
    ...
    filteredBrands = [...filteredBrands]?.filter(
      (brand) =>
        ...
        brand?.code?.toLowerCase()?.includes(lowerCaseSearchTerm) ||
        brand?.desc?.toLowerCase()?.includes(lowerCaseSearchTerm)
    );
  }
  ...
  const handleAddBrand = async () => {
    try {
      ...
      console.error(error);
      toast.error(
        error?.response?.data?.message || "Failed to add brand, try again"
      );
    } finally {
  ...
  const handleEditBrand = async () => {
    openConfirmationModel({
      question: "Are you sure you want to update this brand?",
      answer: ["Yes", "No"],
      onClose: async (result) => {
        if (result) {
          try {
            ...
            await updateBrand(payload, selectedBrand._id);
            dispatch(fetchBrands());
            ...
            toast.error(
              ...
              "Failed to update brand, try again"
            );
          } finally {
            setFormLoading(false);
          }
  ...
  const handleStatusUpdate = async (brand) => {
    openConfirmationModel({
      question: `Are you sure you want to ${brand.status ? "deactivate" : "activate"
        } this brand?`,
      answer: ["Yes", "No"],
      onClose: async (result) => {
        if (result) {
          try {
            ...
            const res = await updateBrand(payload, brand._id);
            dispatch(fetchBrands());
  ...
  const handleExportToCSV = () => {
    const csv = [
      ["BRAND CODE", "BRAND NAME", "BRAND DESCRIPTION", "IMAGE PATH", "STATUS"],
  ...
  const handleCSVImport = async (url) => {
    try {
      dispatch(fetchBrands());
      openConfirmationModel({
        question: "Are you sure you want to import this brand CSV?",
        answer: ["Yes", "No"],
        onClose: async (result) => {
          if (result) {
            try {
              ...
              setFormLoading(true);
              const res = await BrandbulkUpload(payload, "Brand");
              dispatch(fetchBrands());
              if (
                ...
                toast.error(
                  ...
                  } rows imported in the brand Master`
                );
                setErrorLog(res?.data?.skippedRows);
              } else {
                toast.success(
                  `${res?.data?.data?.length} rows imported in the brand Master`
                );
              }
              onCloseModal();
            } catch (error) {
              ...
              toast.error(
                error?.response?.data?.message ||
                "Failed to import brands, try again"
              );
            } finally {
              setFormLoading(false);
            }
  ...
  const handleCSVTemplateDownload = () => {
    const csv = [
      ["BRAND NAME", "BRAND DESCRIPTION", "IMAGE PATH"],
      ["Brand 1", "Brand 1 Description", ""],
      ["Brand 2", "Brand 2 Description", ""],
    ];
  ...
  const handleErrorLogDownload = () => {
    ...
    const csv = [
      ["BRAND NAME", "BRAND DESCRIPTION", "IMAGE PATH", "INDEX", "REASON"],
      ...errorLog.map((data) => [
        data["BRAND NAME"] || "",
    ...
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", "brands-error-log.csv");
    document.body.appendChild(link);
    link.click();
        ...
        <div className="flex justify-start items-center flex-col gap-4 w-full p-4">
          <Card className="w-full flex justify-center items-center flex-col">
            ...
            <div className="w-full flex flex-wrap justify-center items-center gap-2">
              ...
              <Badge color="warning">
                Filtered Count : {filteredBrands?.length}{" "}
              </Badge>
            </div>
            {/* filter div */}
                  ...
                  {filteredBrands.map((brand, index) => (
                    ...
                    >
                      <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-gray-200">
                        <UniqueCode text={brand?.code} codeName="Brand" />
                      </Table.Cell>
                      <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-gray-200">
                        {brand.name}
                      </Table.Cell>
            ...
            <div className="space-y-5">
              ...
              <div className="w-full">
                <Button
                  onClick={
                    modalMode === "add"
                      ? pagePermission?.create && handleAddBrand
                      : pagePermission?.update && handleEditBrand
                  }
