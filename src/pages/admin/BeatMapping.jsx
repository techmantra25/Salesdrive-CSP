...
export const BeatMapping = () => {
  ...
  useEffect(() => {
    ...
    const permission = getPagePermission(permissionState, "beat-mapping");
                        ...
                        >
                          ...
                          <Table.Cell className="font-medium text-gray-900 dark:text-gray-200 text-xs">
                            <div className="flex justify-center items-center gap-2 flex-wrap min-w-80">
                              ...
                              {Array.isArray(employee.regionId)
                                ? employee.regionId.map((region) => (
                                  <span key={region._id}>
                                    {region.name} (
                                    <UniqueCode text={region.code} codeName="Region Code" />)
                                  </span>
                                ))
                                : employee.regionId && (
                                  <span>
                                    {employee.regionId.name} (
                                    <UniqueCode text={employee.regionId.code} codeName="Region Code" />)
                                  </span>
                                )}
                            </div>
                          </Table.Cell>
                          ...
                          <Table.Cell className="font-medium text-gray-900 dark:text-gray-200">
                            {pagePermission?.update && (
                              <EditButton
                                onClick={() => handleEditBeats(employee)}
                                isDisabled={employee.beatId?.length <= 0}
                              />
                            )}
