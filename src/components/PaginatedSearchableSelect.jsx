...
const PaginatedSearchableSelect = ({
  fetchOptions,
  value,
  onChange,
  placeholder,
  ...
  // Direct fetch function for pagination (no debounce)
  ...
  // Debounced search function (only for search input)
  const performSearch = useDebounce(fetchData, 500);

  // Reset options when search term changes
  useEffect(() => {
    if (searchTerm === "") {
  ...
  const handleScroll = (e) => {
    ...
    if (bottom && !loading && hasMore && !loadingRef.current) {
      ...
      setPage(nextPage);
      fetchData(searchTerm, nextPage); // Use direct fetch, not debounced
    }
  };
